...
def filter_routes(data: pl.LazyFrame, distance: int, seats: int) -> pl.LazyFrame:
    ...
    """
    return (
        data.filter(pl.col("GCD") <= distance)
        .filter(pl.col("Seats") <= seats)
        .with_columns(
            [
                pl.when(pl.col("Dep_apt") < pl.col("Arr_apt"))
                .then(pl.col("Dep_apt"))
                .otherwise(pl.col("Arr_apt"))
                .alias("Airport_1"),
                pl.when(pl.col("Dep_apt") < pl.col("Arr_apt"))
                .then(pl.col("Arr_apt"))
                .otherwise(pl.col("Dep_apt"))
                .alias("Airport_2"),
            ]
        )
        .group_by([pl.col("Airport_1"), pl.col("Airport_2")])
        .agg(
            [
                ...
                pl.col("GCD").first(),
                (
                    (pl.col("Seats") * pl.col("Frequency")).sum()
        ...
        .select(
            [
                ...
                pl.col("Arr_apt_lat"),
                pl.col("Arr_apt_lon"),
                pl.col("Airport_1").alias("Dep_apt"),
                pl.col("Airport_2").alias("Arr_apt"),
                pl.col("Number"),
                pl.col("GCD"),
                pl.col("Seats"),
                pl.col("Total_flown"),
...
def fmt(s: float):
    x = format(round(s, 2), ",").replace(",", "x").replace(".", ",").replace("x", ".")
    if "," not in x:
        return str(x) + ",00"
    return x
...
def ifmt(s: float):
    return (
        format(round(s, 2), ",").replace(",", "x").replace(".", ",").replace("x", ".")
    )
